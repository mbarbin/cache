...
let%expect_test "a chain of two: an invalidation travels all the way down" =
  (* @mdexp.code *)
...
let%expect_test "map2 over a computation and a var: each moves it, and both at once" =
  (* @mdexp.code *)
  let cache = Cache.create () in
  let source = ref 1 in
...
let%expect_test
    "an unrelated var write neither triggers a recompute nor stands in for one"
  =
  (* @mdexp.code *)
  let cache = Cache.create () in
  let source = ref 1 in
...
let%expect_test "two computations in one cache stay independent" =
  (* @mdexp.code *)
  let cache = Cache.create () in
...
let%expect_test "collect: invalidating one child computation is picked up" =
  (* @mdexp.code *)
  let cache = Cache.create () in
...
let%expect_test "collect: invalidating one key's computation cuts off at another key" =
  (* @mdexp.code *)
  let cache = Cache.create () in
  let keys = Cache.Var.create cache (Set.of_list (module Int) [ 1; 2 ]) in
  let sources : (int, int ref) Hashtbl.t = Hashtbl.create (module Int) 16 in
...
let%expect_test "collect: the keys node is a computation" =
  (* @mdexp.code *)
  let cache = Cache.create () in
...
let%expect_test "a cutoff absorbs an invalidation that didn't change the value" =
  (* @mdexp.code *)
  let cache = Cache.create () in
...
let%expect_test "a read that recomputes nothing leaves the phase open" =
  (* @mdexp.code *)
  let cache = Cache.create () in
